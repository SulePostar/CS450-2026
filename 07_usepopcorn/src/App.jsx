...
function MovieList({ movies }) { // stateful
  return (
    <ul className="list">
...
function Box({ children }) { // structural
...
function Watched({ movie }) { // presentational
  return (
    <li>
      <img src={movie.Poster} alt={`${movie.Title} poster`} />
      <h3>{movie.Title}</h3>
...
function WatchedList({ watched }) { // presentational
  return (
    <ul className="list">
...
export default function App() { // structural
  ...
  return (
    <>
      <NavBar count={movies.length} onSearch={onSearch} />
      <Main>
        <Box>
